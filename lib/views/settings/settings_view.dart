...
class SettingsView
  ...
  SettingsView({super.key, required super.goRoute})
      : super(
          ...
          coreAppBar: (context, viewModel) => _buildAppBar(context, viewModel, goRoute),
        );

  static AppBar _buildAppBar(
    BuildContext context,
    SettingsCubit viewModel,
    Function(String) goRoute,
  ) {
    return AppBar(
      leading: IconButton(
    ...
    return SingleChildScrollView(
      ...
      child: Column(
        ...
        children: [
          ...
          NotificationsSection(viewModel: viewModel, state: state),
          const SizedBox(height: 24),
          const SectionHeader(
            icon: LucideIcons.lock,
            title: 'Security & Privacy',
          ),
          SecuritySection(viewModel: viewModel, state: state),
          const SizedBox(height: 24),
          const SectionHeader(
            icon: LucideIcons.volume2,
            title: 'Sound & Haptics',
          ),
          SoundSection(viewModel: viewModel, state: state),
          const SizedBox(height: 24),
          const SectionHeader(icon: LucideIcons.wifi, title: 'Network & Data'),
          NetworkSection(viewModel: viewModel, state: state),
          const SizedBox(height: 24),
          const SectionHeader(
          ...
          PerformanceSection(viewModel: viewModel, state: state),
          const SizedBox(height: 32),
          StateDisplaySection(state: state),
          const SizedBox(height: 16),
          RawDataDisplaySection(state: state),
        ],
      ),
    );
  }
